---
layout: post
title:  "My experiments to understand how Python module imports work (or don't)"
comments: true
share: true
tags:
  - self learning
...
Despite using Python for several years, I frequently get confused about how module imports work. I had been putting off properly learning this but a few weeks ago, I decided to finally grab the rose by its thorns. I followed my preferred approach to self-learn by experimenting with codes, reading the "theory" along the way as needed.

I catalog my experiments for my future reference, but it hopefully helps others as well. 

If you're a Python beginner (especially in the modules import topic) and plan to use this tutorial for self-learning, please follow along by _writing_ the source codes (not just "copy pasting" my source codes). Also, perform your own experiments, then pause and reflect upon what you have understood, before the next experiment. Most importantly, document your learning (even if you're not sure your understanding is correct). Even though that may take you ten times longer time, you will learn a lot more and save yourself a lot of headaches in future.

If you're a Python expert, please feel free to critically review this post, and provide your feedback in the comments wherever I have made mistakes. After understanding the mistakes, I will quite happily update the post, with full credit to you.
...
---
### **Experiment no. 1**: [start with Hello World](https://github.com/dragondive/heavens-arena/commit/983001223492b9962aeb036d72356e43455dcfb4)
...
> **Wisdom, i.e. Gyan**: Following the rich traditions of software development.

---
### **Experiment no. 2**: [Call a method from another module in same directory](https://github.com/dragondive/heavens-arena/commit/a2ba4305f1e6e8c723207e513d8a65f524bfa513)
...
---
### **Experiment no. 3**: [Run python from a different directory](https://github.com/dragondive/heavens-arena/commit/74b43a78eb5e97492fcf8c7f633d85441ffcf55c)
...
### **Experiment no. 4**: [Move other module to other directory](https://github.com/dragondive/heavens-arena/commit/66bc623b408364d549c9aba0cf6e009fed873875), then [import only the other directory](https://github.com/dragondive/heavens-arena/commit/471359462e1710046f1133f115b98d3d763a7514)
...
### **Experiment no. 5**: [Import the module from the other directory](https://github.com/dragondive/heavens-arena/commit/54587911affa9231149d51066d6ba2855fc66956)
...
### **Experiment no. 6**: [Create a conflict with a standard module](https://github.com/dragondive/heavens-arena/commit/280185e95f25a886ef46093e1e5efba759fdfcfb) then "resolve" it by [creating an `__init__.py` file](https://github.com/dragondive/heavens-arena/commit/69f396733dedc1d9de2fd057263e76280c634da9)
...
> **Wisdom**:
>  
> _[In experiment no. 3, I had mentioned that it is not fully accurate that the Python interpreter searches for modules in `sys.path`. This is where I properly resolve it.]_
>
...
It wasn't clear to me how creating the `__init__.py` causes the interpreter to give precedence to the "local" module over the built-in module, even after searching on the internet and reading the official documentation for several hours. However, this article [Python behind the scenes #11: how the Python import system works](https://tenthousandmeters.com/blog/python-behind-the-scenes-11-how-the-python-import-system-works/) by [Victor Skvortsov](https://github.com/r4victor) gives the most clear explanation. In particular:
...
### **Experiment no. 7**: [Import a third module from the second module](https://github.com/dragondive/heavens-arena/commit/70631f5aff85e54e848825964bd3036a4d3e1bec)
...
>  
> However, this experiment induced me to properly learn the following:
> * Circular import
> * The difference between `import X` and `from X import Y`

---

### **Experiment no. 8**: [Import from a deeper level directory](https://github.com/dragondive/heavens-arena/commit/7a7dc3f9716e43781b3e15b202122be9a613cac6)
...
### **Experiment no. 9**: [Create a circular import](https://github.com/dragondive/heavens-arena/commit/577b32b272fe7ac4558911e6bdaa1084c81b5b97)

**NOTE: I must emphasize *again*, in case it is not already clear from the context, that this should not be done in a production environment.** I create circular imports in this and subsequent experiments only to improve my understanding of how the module import works. As you will notice, this has also helped me to properly understand (at least to some extent) the difference between `import X` and `from X import Y`; and about how the interpreter handles the module table.
...
```python
# NOTE: Omitting directory names for clarity of understanding
# main.py
import message_provider  # >-----+
                         #   [1] |
# message_provider.py    # <-----+    <-----------------+
...
### **Experiment no. 10**: [Resolve the circular import by importing entire module](https://github.com/dragondive/heavens-arena/commit/31ff70bfc4326c60903cd4338e27e5939d950069)

The source code change from experiment no. 9 is that in `answer_provider.py`, the full module `message.message_provider` is imported, not only `get_real_message`. This resolves the circular import as explained below:
...
### **Experiment no. 11**: [Second module fully includes third module, but third module partially includes second module](https://github.com/dragondive/heavens-arena/commit/dc88034b21a858ee3b3e50f26f6e2592f343b05c)

I did this experiment mainly to "verify" my learning from experiment no. 9. The only difference is `message_provider` now fully `import`s `answer_provider`, and not only the `get_answer` method. Following the same "execution" flow as in experiment no. 9, I figured out this should _also_ lead to a circular import, and it did.
...
### **Experiment no. 12**: ["Hack" to resolve the circular import by putting the import statement at end of the module](https://github.com/dragondive/heavens-arena/commit/2b5d51fbe157c3a040ac4f7482b8ffc2b9ad12ba)

This was another experiment I did mainly to "verify" my learning from experiment no. 9. In `message_provider`, I placed the `import answer.deep.answer_provider` statement at the end of the module. I expected that this should resolve the circular import, and indeed, it did.
...
> **Wisdom**:
> 
> In experiment no. 9, the circular import occured when the interpreter was "executing" `answer_provider` module. It tried to import `message.message_provider.get_real_message`, which it did not know yet. However, when the `import` statement is placed at the end, the interpreter already knows this method before it starts "executing" `answer_provider` module, hence circular import does not occur. 

---

### **Experiment no. 13**: [Always import full module to fix the circular import](https://github.com/dragondive/heavens-arena/commit/708a11bc36bbfc6bae08cb936a24c8bf21212da6)

This was the final experiment to conclude (for now) my learning from experiment no. 9. I changed the `import` statements everywhere to import the full module, and as expected, this did not cause any circular imports.
...
I thought to explore these topics to get better understanding, but I didn't want to risk losing my learning by waiting too long to write it down. Hence, I decided to push this first installment of Gyan out of the door, and I will (try to) write about the other topics in a separate post later.